...
3. Просматриваю список сетевых интерфейсов командой ```ifconfig```. \
...
4. Получаю ip адрес от dhcp сервера командой: ```sudo dhclient -v``` \
...
5. Определяю внешний IP адрес шлюза командой: ```wget -O - -q icanhazip.com``` \
  ![Outside IP](img/part3_outsideaddr.png) \
	*Внешний IP адрес шлюза.*

6. Определяю внутренний IP адрес шлюза командой: ```route -n``` \
  ![Inside IP](img/part3_insideaddr.png) \
	*Внутренний IP адрес шлюза.*

7. Задаю статичные ip, gateway и dns, изменяя файл конфигурации интерфейса. ```sudo nano /etc/netplan/00-installer-config.yaml```
  ![Static config](img/part3_defaultconf.png) \
	...
	*Измененная конфигурация.* 
	...
	После внесеннных изменений применим их командой ```sudo netplan apply```. \
	![Check IP](img/part3_getip_with_conf.png) \
	*Проверка IP адреса после внесенных изменений.* \

	Перезагружаем машину командой ```sudo reboot``` и пингуем сервера.
